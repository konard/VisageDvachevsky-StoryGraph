...
public:
  ...
  std::string getTestPath() const { return testDir.string(); }
...
bool savesAreEqual(const SaveData& a, const SaveData& b) {
  if (a.sceneId != b.sceneId || a.nodeId != b.nodeId)
    return false;
  if (a.intVariables != b.intVariables)
    return false;
  if (a.floatVariables != b.floatVariables)
    return false;
  if (a.flags != b.flags)
    return false;
  if (a.stringVariables != b.stringVariables)
    return false;
  if (a.thumbnailWidth != b.thumbnailWidth || a.thumbnailHeight != b.thumbnailHeight)
    return false;
  if (a.thumbnailData != b.thumbnailData)
    return false;
  return true;
}
...
TEST_CASE("SaveManager slotExists returns false for non-existent slots",
          "[save_manager][slot_management]") {
  SaveManagerTestFixture fixture;
  SaveManager manager;
  manager.setSavePath(fixture.getTestPath());
...
TEST_CASE("SaveManager save with compression enabled", "[save_manager][compression]") {
  ...
  SaveData data = createTestSaveData();
  for (int i = 0; i < 100; i++) {
    data.stringVariables["repeated_key_" + std::to_string(i)] =
        "repeated_value_repeated_value_repeated_value";
  }
...
TEST_CASE("SaveManager detect corrupted file - checksum mismatch", "[save_manager][corruption]") {
  ...
  std::vector<NovelMind::u8> fileContents((std::istreambuf_iterator<char>(readFile)),
...
TEST_CASE("SaveManager load legacy v1 format", "[save_manager][migration]") {
  ...
  for (char c : sceneId)
    checksum = checksum * 31 + static_cast<NovelMind::u32>(c);
  for (char c : nodeId)
    checksum = checksum * 31 + static_cast<NovelMind::u32>(c);
  for (char c : intName)
    checksum = checksum * 31 + static_cast<NovelMind::u32>(c);
