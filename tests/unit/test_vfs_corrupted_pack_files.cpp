...
TEST_CASE("PackReader - Truncated pack file (partial header)",
          "[vfs][pack][corruption][truncated]") {
  PackReader reader;
...
TEST_CASE("PackIntegrityChecker - CRC mismatch detection", "[vfs][pack][corruption][crc]") {
  ...
  SECTION("Detect data corruption via CRC mismatch") {
    ...
    auto result = checker.verifyResource(reinterpret_cast<const u8*>(corruptedData),
                                         std::strlen(corruptedData), 0, std::strlen(corruptedData),
                                         originalCrc);
  ...
  SECTION("CRC verification passes with matching data") {
    ...
    u32 crc =
        PackIntegrityChecker::calculateCrc32(reinterpret_cast<const u8*>(data), std::strlen(data));

    auto result = checker.verifyResource(reinterpret_cast<const u8*>(data), std::strlen(data), 0,
                                         std::strlen(data), crc);
  ...
  SECTION("Single bit flip detected by CRC") {
    ...
    auto result = checker.verifyResource(data.data(), data.size(), 0, data.size(), originalCrc);
...
TEST_CASE("PackReader - Error recovery and stability", "[vfs][pack][corruption][recovery]") {
  ...
  SECTION("No crashes with various corrupted inputs") {
    ...
    std::vector<std::string> testFiles = {"test_empty.pack", "test_partial.pack",
                                          "test_random.pack"};
