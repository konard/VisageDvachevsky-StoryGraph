...
std::vector<NovelMind::u8> createBytecodeHeader(NovelMind::u32 magic, NovelMind::u16 version,
                                                NovelMind::u16 flags, NovelMind::u32 instrCount,
...
void appendInstruction(std::vector<NovelMind::u8>& bytecode, NovelMind::u8 opcode,
...
TEST_CASE("Interpreter deserialize max int - valid case", "[interpreter][security]") {
  ...
  auto bytecode = createBytecodeHeader(SCRIPT_MAGIC,
                                       1,                     // version
                                       0,                     // flags
...
TEST_CASE("Interpreter deserialize overflow - instruction count exceeds max",
  ...
  auto bytecode = createBytecodeHeader(SCRIPT_MAGIC, 1, 0,
...
TEST_CASE("Interpreter deserialize overflow - instruction count would overflow multiplication",
          "[interpreter][security]") {
  ScriptInterpreter interpreter;
  ...
  if (static_cast<NovelMind::usize>(dangerousInstrCount) > maxSafeSize) {
    auto bytecode = createBytecodeHeader(SCRIPT_MAGIC, 1, 0, dangerousInstrCount, 0, 0, 0);
...
TEST_CASE("Interpreter deserialize overflow - addition overflow check", "[interpreter][security]") {
  ...
  NovelMind::usize maxSafeSize = std::numeric_limits<NovelMind::usize>::max() / 5;
  NovelMind::u32 largeButSafeInstrCount = static_cast<NovelMind::u32>(std::min(
      maxSafeSize - 1, static_cast<NovelMind::usize>(std::numeric_limits<NovelMind::u32>::max())));

  auto bytecode = createBytecodeHeader(SCRIPT_MAGIC, 1, 0, largeButSafeInstrCount, 0, 0, 0);
...
TEST_CASE("Interpreter deserialize overflow - string count exceeds max",
          "[interpreter][security]") {
  ScriptInterpreter interpreter;
...
TEST_CASE("Interpreter deserialize - bytecode truncated at instruction count",
          "[interpreter][security]") {
  ScriptInterpreter interpreter;
...
TEST_CASE("Interpreter deserialize - bytecode truncated at string count",
          "[interpreter][security]") {
  ScriptInterpreter interpreter;
...
TEST_CASE("Interpreter deserialize - invalid magic number", "[interpreter][security]") {
  ...
  auto bytecode = createBytecodeHeader(0xDEADBEEF, // Invalid magic
                                       1, 0, 0, 0, 0, 0);
