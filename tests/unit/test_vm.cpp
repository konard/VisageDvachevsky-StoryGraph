...
TEST_CASE("VM stack overflow error message verification", "[scripting][recursion][security]") {
  ...
  TEST_CASE("test_vm_store_var_empty_stack", "[scripting][stack_underflow]") {
    ...
    std::vector<Instruction> program = {{OpCode::STORE_VAR, 0}, // Need 1 element on stack
                                        {OpCode::HALT, 0}};
  ...
  TEST_CASE("test_vm_jump_if_empty_stack", "[scripting][stack_underflow]") {
    ...
    SECTION("JUMP_IF with empty stack") {
      std::vector<Instruction> program = {{OpCode::JUMP_IF, 2}, // Need 1 element on stack
                                          {OpCode::HALT, 0}};
      vm.load(program, {});
      vm.run();
      REQUIRE(vm.isHalted());
    ...
    SECTION("JUMP_IF_NOT with empty stack") {
      std::vector<Instruction> program = {{OpCode::JUMP_IF_NOT, 2}, // Need 1 element on stack
                                          {OpCode::HALT, 0}};
      vm.load(program, {});
      vm.run();
      REQUIRE(vm.isHalted());
  ...
  TEST_CASE("test_vm_normal_operations_after_fix", "[scripting][stack_underflow]") {
    ...
    std::vector<Instruction> program = {
        {OpCode::PUSH_INT, 10}, {OpCode::PUSH_INT, 5},  {OpCode::ADD, 0}, {OpCode::PUSH_INT, 3},
        {OpCode::MUL, 0},       {OpCode::STORE_VAR, 0}, {OpCode::HALT, 0}};
