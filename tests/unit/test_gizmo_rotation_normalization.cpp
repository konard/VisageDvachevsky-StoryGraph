...
TEST_CASE("NMTransformGizmo normalizes rotation at drag start",
          "[gizmo][rotation][normalization]") {
  int argc = 0;
  char* argv[] = {nullptr};
...
TEST_CASE("NMTransformGizmo prevents drift after multiple drags", "[gizmo][rotation][drift]") {
  ...
  SECTION("Full rotation cycles maintain consistency") {
    ...
    for (int i = 0; i < steps; ++i) {
      ...
      QPointF dragStart =
          center + QPointF(50 * std::cos(currentAngle), 50 * std::sin(currentAngle));
