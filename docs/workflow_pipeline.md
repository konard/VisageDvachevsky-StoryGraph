...
> **См. также:** [WORKFLOW_MODES.md](WORKFLOW_MODES.md) — подробная документация
> по режимам работы на английском языке.
...
### 7.8 Структура папки `scripts/generated/`

Папка `scripts/generated/` предназначена для автоматически сгенерированных
скриптов из Story Graph:

```
scripts/
├── main.nms              # Основной скрипт (редактируется вручную)
└── generated/            # Автоматически сгенерированные скрипты
    ├── .gitkeep          # Placeholder для git
    └── (scene_name.nms)  # Генерируются при синхронизации
```

**Режим Script:** Папка `generated/` остаётся пустой — все изменения в `main.nms`.

**Режим Graph:** Папка `generated/` содержит экспорт из Story Graph после
синхронизации.

**Режим Mixed:** Папка `generated/` используется как резервная копия Graph-контента.

### 7.9 Типичные проблемы и решения

#### Проблема: Изменения в `main.nms` не применяются

**Возможные причины:**
1. Проект в режиме Graph или Mixed (Graph имеет приоритет)
2. Сцена с таким же именем существует в Story Graph

**Решение:**
1. Проверьте режим в Project Settings или Play Toolbar
2. Переключитесь в режим Script, если нужно использовать скрипты
3. Убедитесь, что имена сцен уникальны

#### Проблема: Папка `scripts/generated` пустая

**Причина:** Это нормальное поведение в режиме Script.

**Решение:**
1. Если вы работаете в Graph/Mixed режиме, нажмите "Sync to Script"
2. Файлы появятся после синхронизации

#### Проблема: Линтер показывает ошибки `goto` для сцен из Graph

**Причина:** Линтер валидирует только скрипты, а не содержимое Graph.

**Решение:**
1. Используйте "Sync Graph to Script" для экспорта сцен в скрипты
2. Или игнорируйте предупреждения, если сцены существуют только в Graph
3. В Mixed режиме — убедитесь, что Graph и Script синхронизированы
