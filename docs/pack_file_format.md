...
| 0x04 | 32 | uint8[32] | SHA-256 заголовка + таблицы ресурсов + таблицы строк (криптографическая проверка целостности) |
| 0x24 | 4 | uint32 | CRC32 заголовка + таблицы ресурсов + таблицы строк (быстрая проверка повреждений) |
| 0x28 | 8 | uint64 | Временная метка создания пакета (Unix epoch) |
| 0x30 | 4 | uint32 | Номер сборки |
| 0x34 | 12 | uint8[12] | Зарезервировано (должно быть нулем) |
...
2. Чтение footer и проверка SHA-256 таблицы (криптографическая проверка целостности)
3. Опционально: быстрая проверка CRC32 таблицы (обнаружение случайных повреждений)
4. Загрузка таблицы ресурсов в память
5. Загрузка таблицы строк в память
6. Построение карты ID ресурса -> запись
7. При запросе ресурса:
   ...
   e. Проверка CRC32 (быстрое обнаружение повреждений)
...
Footer (64 байта)
  Magic: "NMRF"
  SHA-256: <32 байта хеша таблиц для криптографической проверки целостности>
  CRC32: <4 байта для быстрого обнаружения повреждений>
  ...
```
