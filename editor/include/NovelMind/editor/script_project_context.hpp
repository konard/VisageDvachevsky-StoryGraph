...
public:
  ...
  [[nodiscard]] bool
  backgroundExists(const std::string &assetId) const override;
  [[nodiscard]] bool audioExists(const std::string &assetPath,
                                 const std::string &mediaType) const override;
  [[nodiscard]] bool
  characterSpriteExists(const std::string &characterId) const override;
  ...
  getAvailableAudio(const std::string &mediaType) const;
...
private:
  ...
  [[nodiscard]] bool fileExistsWithExtensions(
  ...
  [[nodiscard]] std::vector<std::string>
  getFilesInDirectory(const QString &directory,
                      const std::vector<QString> &extensions) const;
};
