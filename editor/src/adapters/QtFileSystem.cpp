...
bool QtFileSystem::writeFile(const std::string& path, const std::string& content) {
  QFile file(QString::fromStdString(path));
...
bool QtFileSystem::writeBinaryFile(const std::string& path, const std::vector<u8>& data) {
  QFile file(QString::fromStdString(path));
...
bool QtFileSystem::copyFile(const std::string& src, const std::string& dest) {
  ...
  return QFile::copy(QString::fromStdString(src), QString::fromStdString(dest));
}

bool QtFileSystem::moveFile(const std::string& src, const std::string& dest) {
  return QFile::rename(QString::fromStdString(src), QString::fromStdString(dest));
}
...
std::vector<std::string> QtFileSystem::listFiles(const std::string& directory,
...
std::vector<std::string> QtFileSystem::listDirectories(const std::string& directory) const {
  QDir dir(QString::fromStdString(directory));
  if (!dir.exists()) {
    return {};
  }
...
std::vector<std::string> QtFileSystem::listFilesRecursive(const std::string& directory,
                                                          const std::string& filter) const {
  std::vector<std::string> result;
  ...
  QDirIterator it(QString::fromStdString(directory), filters, QDir::Files,
...
FileInfo QtFileSystem::getFileInfo(const std::string& path) const {
  ...
  result.lastModified = static_cast<u64>(info.lastModified().toMSecsSinceEpoch());
...
u64 QtFileSystem::getLastModified(const std::string& path) const {
  QFileInfo info(QString::fromStdString(path));
  return info.exists() ? static_cast<u64>(info.lastModified().toMSecsSinceEpoch()) : 0;
}
...
std::string QtFileSystem::joinPath(const std::string& base, const std::string& component) const {
