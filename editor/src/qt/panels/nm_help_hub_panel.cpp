...
NMHelpHubPanel::NMHelpHubPanel(QWidget* parent) : NMDockPanel(tr("Help Hub"), parent) {
  setPanelId("help_hub");
  setupUi();
  createConnections();
}
...
void NMHelpHubPanel::createConnections() {
  connect(m_searchEdit, &QLineEdit::textChanged, this, &NMHelpHubPanel::filterTutorials);

  connect(m_levelFilter, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &NMHelpHubPanel::filterByLevel);

  connect(m_tutorialTree, &QTreeWidget::itemClicked, this, &NMHelpHubPanel::onTutorialItemClicked);

  connect(m_tutorialTree, &QTreeWidget::itemDoubleClicked, this,
          &NMHelpHubPanel::onTutorialItemDoubleClicked);

  connect(m_startButton, &QPushButton::clicked, this, &NMHelpHubPanel::onStartButtonClicked);

  connect(m_resetProgressButton, &QPushButton::clicked, this,
          &NMHelpHubPanel::onResetProgressButtonClicked);

  connect(m_disableButton, &QPushButton::clicked, this, &NMHelpHubPanel::onDisableButtonClicked);

  connect(m_resetAllButton, &QPushButton::clicked, this,
          &NMHelpHubPanel::onResetAllProgressClicked);

  connect(m_settingsButton, &QPushButton::clicked, this, &NMHelpHubPanel::onSettingsButtonClicked);

  // Connect to tutorial manager for updates
  if (NMTutorialSubsystem::hasInstance()) {
    connect(&NMTutorialManager::instance(), &NMTutorialManager::progressChanged, this,
            &NMHelpHubPanel::refreshTutorialList);
  }
}
...
void NMHelpHubPanel::filterTutorials(const QString& searchText) {
  ...
  for (int i = 0; i < m_tutorialTree->topLevelItemCount(); ++i) {
    ...
    for (int j = 0; j < categoryItem->childCount(); ++j) {
      ...
      bool matches = tutorialItem->text(0).contains(searchText, Qt::CaseInsensitive);
...
void NMHelpHubPanel::filterByLevel(int levelIndex) {
  ...
  for (int i = 0; i < m_tutorialTree->topLevelItemCount(); ++i) {
    ...
    for (int j = 0; j < categoryItem->childCount(); ++j) {
      ...
      auto tutorial = NMTutorialManager::instance().getTutorial(tutorialId.toStdString());
      if (tutorial) {
...
void NMHelpHubPanel::updateTutorialDetails() {
  ...
  auto tutorial = NMTutorialManager::instance().getTutorial(m_selectedTutorialId.toStdString());
  if (!tutorial) {
  ...
  if (!NMTutorialManager::instance().arePrerequisitesMet(tutorial->id)) {
    m_startButton->setEnabled(false);
    m_progressLabel->setText(m_progressLabel->text() + tr("\n(Prerequisites not met)"));
  }
}
...
void NMHelpHubPanel::onStartButtonClicked() {
  ...
  // Start the tutorial
  bool started = NMTutorialSubsystem::instance().startTutorial(m_selectedTutorialId.toStdString());
...
void NMHelpHubPanel::onResetProgressButtonClicked() {
  ...
  auto reply = NMMessageDialog::showQuestion(
      this, tr("Reset Progress"), tr("Are you sure you want to reset progress for this tutorial?"),
